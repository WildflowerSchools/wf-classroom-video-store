...
@click.option(
    ...
    help="env file to load environment_name variables from. Defaults to .env",
)
def cli(env_file):
    if env_file is None:
...
@click.option(
    ...
    help="Number of days of videos to retain. Note if 0 is provided, the current day"
    "s videos will be retained (i.e. retention_days doesn"
    "t include today). Anything older than the provided value will be permanently removed.",
)
@click.option(
    "--environment-id",
    "environment_ids",
    default=[],
    multiple=True,
    help="Environment ID to run deletions against. If none provided, videos across all environments will be removed",
)
...
def delete_video(retention_delta, environment_ids, dry):
    ...
    for environment in environments:
        ...
        expiration_datetime = pytz.timezone(environment["timezone_name"]).localize(
            datetime.combine(datetime.today(), time.max)
